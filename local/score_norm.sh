...
if [ $stage -le 2 ] && [ $stop_stage -ge 2 ]; then
  ...
  for x in $trials; do
    python wespeaker/bin/score_norm.py \
      ...
      --eval_emb_scp ${exp_dir}/embeddings/vnceleb_test/xvector.scp \
      --mean_vec_path ${exp_dir}/embeddings/vnceleb_train/mean_vec.npy
  done
fi
...
if [ $stage -le 3 ] && [ $stop_stage -ge 3 ]; then
  ...
  for x in ${trials}; do
    ...
    python wespeaker/bin/compute_metrics.py \
      ...
      2>&1 | tee -a ${scores_dir}/vnceleb_${score_norm_method}${top_n}_result
